...
if __name__ == "__main__":
    try:
        # Load Excel sheets
        lauf = pd.read_excel(EXCEL_PATH, sheet_name=SHEET_LAUF, header=START_ROW)
        verkn = pd.read_excel(EXCEL_PATH, sheet_name=SHEET_VERKN, header=2)
        zugliste = pd.read_excel(EXCEL_PATH, sheet_name=SHEET_ZUG, header=0)
        ...
        print(f"Saved in {html_filename}")

    except Exception as e:
        import traceback
        print("❌ Error in TimeScheduleGenerator.py:")
